...
Deno.serve(async request => {
  ...
  if (!targetId || typeof payload.is_active !== 'boolean') return json({ error: 'Choose an account and status.' }, 400);
  if (targetId === user.id && !payload.is_active) return json({ error: 'You cannot disable your own account.' }, 400);
